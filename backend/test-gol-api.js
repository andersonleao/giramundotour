...
(async () => {
    ...
    console.log(`[${e()}] POST /gol-lookup com origin REC...`);
    const start = await fetch(`${BASE}/api/reservas/gol-lookup`, {
        method: 'POST', headers: h(),
        body: JSON.stringify({ pnr: 'SEOBQV', origin: 'REC', lastName: 'SILVA' })
    ...
    for (let i = 1; i <= 65; i++) {
        ...
        if (data.status === 'done') {
            ...
            console.log(`\n✅ SUCESSO em ${e()}`);
            console.log(`Passageiro: ${bd?.passageiroNome}`);
            console.log(`Tipo: ${bd?.tripType}`);
